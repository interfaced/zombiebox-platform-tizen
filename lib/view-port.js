...
export default class ViewPort extends AbstractViewPort {
	...
	constructor(avplay, videoObject, panelResolution, appResolution, version) {
		...
		console.log(`[Viewport] panel: ${panelResolution.name}; app: ${appResolution.name}; version: ${version}`);
	...
	updateViewPort() {
		...
		// eslint-disable-next-line no-console
		console.log('[Viewport] updateViewPort', {
			'avplay state': this._avplay.getState(),
			'area': currentArea.getValue(),
			'isFullScreen': this.isFullScreen(),
			'aspectRatio': this._aspectRatio.explain()
		});
