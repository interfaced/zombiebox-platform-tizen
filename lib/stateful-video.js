...
export default class StatefulVideo extends AbstractStatefulVideo {
	...
	prepare(url, options = {}) {
		...
		const prepare = () => {
			...
			if (PrepareOption.START_POSITION in options) {
				this._fireEvent(this.EVENT_DEBUG_MESSAGE, 'starting at ' + (options[PrepareOption.START_POSITION]));

				// Despite what docs claim this operation seems to be synchronous in IDLE state
				// and callbacks are never actually called
				this._avplay.seekTo(
					/** @type {number} */ (options[PrepareOption.START_POSITION]),
					() => this._fireEvent(this.EVENT_DEBUG_MESSAGE, 'seekto callback')
				);
			}
	...
	_createAVPlayListener() {
		const handlers = {
			...
			// Undocumented:
			'onhttperrorevent': undefined,
			'onuserdata': undefined
		};

		for (const [key, handler] of Object.entries(handlers)) {
			handlers[key] = (...params) => {
				this._handleAVPlayEvent(key, handler, ...params);
			};
		}

		handlers.onerror = (errorText) => this._onError(new Error(errorText));
	 ...
	 * @protected
	 */
	_handleAVPlayEvent(event, handler, ...params) {
		this._fireEvent(
			this.EVENT_DEBUG_MESSAGE,
			`avplay ${event}: [${params.map((p) => JSON.stringify(p)).join(', ')}]`
		);
		...
		if (
			...
			this._fireEvent(
				this.EVENT_DEBUG_MESSAGE,
				`avplay ${event} ignored because video is in ${this._stateMachine.getCurrentState()} state`
			);
			return;
		}
		...
		if (!this._avplayCallbackActive) {
			this._fireEvent(
				this.EVENT_DEBUG_MESSAGE,
				`avplay ${event} ignored because callback is not active`
			);
			return;
		}
		...
		if (handler) {
			this._failToErrorState(handler.bind(this, ...params));
		}
	}
	...
	_onAVPlayBufferingStart() {
		...
		// This is just a mystery, what the hell is it even buffering when no media source is known?
