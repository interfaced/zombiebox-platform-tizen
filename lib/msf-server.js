...
export default class MSFServer extends EventPublisher {
	 ...
	 * @param {*=} message
	 */
	sendEvent(target, event, message) {
		this._assertChannel();

		this._channel.publish(event, message, typeof target === 'string' ? target : target.id);
	}
