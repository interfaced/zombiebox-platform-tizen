...
export default class Video extends AbstractVideo {
	 ...
	 * @param {IInfo} info
	 */
	constructor(videoContainer, info) {
	...
	play(url, opt_position) {
		this._taskManager.addTask(
			this._createPlayTask(url, opt_position),
			TaskType.PLAY
		);
	}
	 ...
	 * @protected
	 */
	_createPlayTask(url, opt_position) {
		return () => new Promise((resolve, reject) => {
			const play = () => {
				try {
					this._play();
			...
			const onPreparationDone = () => {
				...
				if (opt_position) {
					try {
						this._plugin.seekTo(opt_position, play, reject);
					} catch (e) {
						reject();
					}
				} else {
	...
	_initVideoObject() {
		...
		this._videoContainer.appendChild(this._videoObject);
	}
	 ...
	 */
	_deinitVideoObject() {
		this._videoContainer.removeChild(this._videoObject);
	}
